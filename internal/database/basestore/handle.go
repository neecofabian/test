...
func NewHandleWithDB(db *sql.DB, txOptions sql.TxOptions) TransactableHandle {
	return &dbHandle{
		DB:        db,
		logger:    log.With().Str("internal", "database").Logger(),
		txOptions: txOptions,
	}
}
...
func (h *txHandle) Done(err error) error {
	...
	return errors.Join(err, h.Rollback())
}
...
func (t *lockingTx) lock() {
	if !t.mu.TryLock() {
		...
		err := errors.Join(ErrConcurrentTransactionAccess)
		t.logger.Error().Msgf("transaction used concurrently %s", err)
		t.mu.Lock()
	}
}
